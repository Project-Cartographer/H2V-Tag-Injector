...
namespace global_handle_function
{
	...
	{
		return	((HANDLE(__cdecl *)(const char*))((char*)game.GetBase() + 0x38607))(pScenario);
	}
}
...
namespace global_objects_fix
{
	void __cdecl bipd_fix(unsigned __int16 datum_index)
	{
		...
		sub_EC23F = (void(_cdecl*)(unsigned __int16))((char*)game.GetBase() + 0x1389B0);
	...
	void __cdecl crea_fix(unsigned __int16 datum_index)
	{
		...
		sub_EC23F = (int(_cdecl*)(unsigned __int16))((char*)game.GetBase() + 0x138985);
		sub_EC23F(datum_index);
	}
	void __cdecl vehi_fix(unsigned __int16 datum_index)
	{
		int(_cdecl*sub_EC23F)(unsigned __int16);
		sub_EC23F = (int(_cdecl*)(unsigned __int16))((char*)game.GetBase() + 0x13895A);
		sub_EC23F(datum_index);
	}
	void __cdecl coll_fix(unsigned __int16 datum_index)
	{
		int(_cdecl*sub_EC23F)(unsigned __int16);
		sub_EC23F = (int(_cdecl*)(unsigned __int16))((char*)game.GetBase() + 0x7BE5C);
		sub_EC23F(datum_index);
	}
	void __cdecl phmo_fix(unsigned __int16 datum_index)
	{
		int(_cdecl*sub_EC23F)(unsigned __int16, int);
		sub_EC23F = (int(_cdecl*)(unsigned __int16, int))((char*)game.GetBase() + 0x7B844);
...
namespace tag_loader
{
	...
	void Load_RAW_refs(int datum_index,string map_loc)
	{
		...
		*(HANDLE*)(game.GetBase() + 0x4AE8A8) = new_file_handle;
		...
		case 0x6D6F6465://mode
			...
			if (*(int*)(tag_mem_addr + 0x24) > 0)
			{
				...
				do
				{
					...
					((void(__cdecl *)(int, unsigned int))((char*)game.GetBase() + 0x2652BC))(sections_base + off + 0x38, 3u);
					++v15;
					off += 0x5C;
		...
		case 0x6269746D://bitm
		{
			...
			int old_list_field = *(DWORD*)(game.GetBase() + 0xA49270 + 0x1FC);
			...
			for (int i = 0; i < *(int*)(tag_mem_addr + 0x44); i++)
			{
				...
				*(DWORD*)(game.GetBase() + 0xA49270 + 0x1FC) = bitmaps_field;

				int temp = 0;
				((int(__cdecl *)(int, char, int, void*))((char*)game.GetBase() + 0x265986))(bitmaps_field, 2, 0, &temp);

				((int(__cdecl *)(int, char, int, void*))((char*)game.GetBase() + 0x265986))(bitmaps_field, 0, 0, &temp);

			}
			*(DWORD*)(game.GetBase() + 0xA49270 + 0x1FC) = old_list_field;
			break;
		}
		default:
			break;
...
unsigned int __cdecl AllocateMemory(int old_size, char arg_4)
{
	...
	pAllocate_memory = (Allocate_memory)((char*)game.GetBase() + 0x37E69);
...
char _cdecl LoadTagsandMapBases(int a)
{
	...
	*TagTableStart = (DWORD)tag_loader::new_Tables;
	...
	tag_loader::tag_count = *(DWORD*)(*(DWORD*)(game.GetBase() + 0x47D568) + 0x18);
	...
	*/
	...
	tag_loader::Parse_query_file(tag_loader::dll_dir + "\\tags\\load_tags.txt");

	string message = tag_loader::Pop_messages();
	pLog.WriteLog(message.c_str());

	return result;
}
void _Patch_calls()
{
	PatchCall(game.GetBase() + 0x313B2, (DWORD)AllocateMemory);//allocating more space for meta loading
	PatchCall(game.GetBase() + 0x3166B, (DWORD)LoadTagsandMapBases);//default maps meta loading
	PatchCall(game.GetBase() + 0x315ED, (DWORD)LoadTagsandMapBases);//custom maps meta loading,i know i am taking risks
}
